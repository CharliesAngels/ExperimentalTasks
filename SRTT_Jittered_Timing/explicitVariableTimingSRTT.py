...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.81.03), Mon May 22 14:01:06 2017
...
WhiteCross = visual.TextStim(win=win, ori=0, name='WhiteCross',
    ...
    color='white', colorSpace='rgb', opacity=1,
    depth=-1.0)
BlockType = visual.TextStim(win=win, ori=0, name='BlockType',
...
Blocks = data.TrialHandler(nReps=1, method='sequential', 
    ...
    trialList=data.importConditions(u'explicitVariableSRT.xlsx'),
...
for thisBlock in Blocks:
    ...
    BlockType.setText(BlockType)
    # keep track of which components have finished
    InitialPauseComponents = []
    InitialPauseComponents.append(WhiteCross)
    InitialPauseComponents.append(BlockType)
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        # *BlockType* updates
        if t >= 8 and BlockType.status == NOT_STARTED:
            # keep track of start time/frame for later
            BlockType.tStart = t  # underestimates by a little under one frame
            BlockType.frameNStart = frameN  # exact frame index
            BlockType.setAutoDraw(True)
        if BlockType.status == STARTED and t >= (8 + (2-win.monitorFramePeriod*0.75)): #most of one frame period left
            BlockType.setAutoDraw(False)
