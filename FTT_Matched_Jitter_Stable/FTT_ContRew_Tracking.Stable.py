...
#            self.curConstant = 25
    ...
    def loadEarning(self,block):
        if self.run in ['001','005'] and block == 0:
            self.earning = 0
        else:
    ...
    def openBlockFile(self,block):
        f = open(self.path+self.subjectID+'.run_'+self.run+'_block_'+str(block)+'.datafile.csv','a')
        return f
...
class expPresentation:
    def __init__(self,experiment):        
        ...
        pos=(0,experiment.origin), size=(0.5,0.5),color='Lime', autoLog=False)
...
#            YZ = visual.TextStim(win=experiment.myWin, text='y_zeroed = '+str(y_zeroed), wrapWidth=10.0, font='Arial',
#            pos=[0, 2], height=1.0, color=[1.000,1.000,1.000], colorSpace='rgb', opacity=1, depth=0.0)
#            YZ.draw()
#            CUR.draw()
...
#            self.InstructionRunL1 = visual.TextStim(win=experiment.myWin, text='Track the blue circle with the white dot.  Squeeze the handle to move up.', wrapWidth=10.0, font='Arial',
#            pos=[0, 1], height=1.0, color=[1.000,1.000,1.000], colorSpace='rgb', opacity=1, depth=0.0)
#            # for earning versions : self.InstructionRunL2 = visual.TextStim(win=experiment.myWin, text='Your tracking performance will determine how much money you earn. The green dot does not indicate your performance.', wrapWidth=10.0, font='Arial',
#            self.InstructionRunL2 = visual.TextStim(win=experiment.myWin, text='The green dot does not indicate your performance.', wrapWidth=10.0, font='Arial',
#            pos=[0, 0], height=1.0, color=[1.000,1.000,1.000], colorSpace='rgb', opacity=1, depth=0.0)
#            self.InstructionRunL3 = visual.TextStim(win=experiment.myWin, text='Press Space to continue.', wrapWidth=10.0, font='Arial',
#            pos=[0, -1], height=1.0, color=[1.000,1.000,1.000], colorSpace='rgb', opacity=1, depth=0.0)
#        while showInst == True:
...
#            self.breakPhraseL2 = ''
#        else:
#            self.breakPhraseL2 = str("You have earned money.")
    ...
    def presentRun(self,experiment):
        ...
        if any([experiment.run in ['001','002','003','004','005'], experiment.run in ['006','007','008','009'] and experiment.congruent == 'Same']):
...
#                    if experiment.run == '001':
#                        if block < experiment.numBlocks - 1:
#                        # changing the weight of the cursor here for jitter period. ##########
#                            multi = random.random()
#                            experiment.cur_weight = experiment.setWeight + multi*12
#                            f = open(experiment.path+experiment.subjectID+'.run_'+experiment.run+'_block_'+str(block)+'_trial_'+str(trial)+'.Jitter.txt','w')
#                            f.write(str(experiment.cur_weight))
#                            f.close()
#                        else: 
#                        #
#                            experiment.cur_weight = experiment.setWeight
#                        #
...
#                T_writer.writerow(btimes)
            ...
            while block < experiment.numBlocks:
                ...
                #print 'P = ' + str(Ps)
                while trial < experiment.numTrials:
                    errorList = self.presentTrials(experiment,As,Ps,criterion,errorList,trial,B_file,block)
                    bErrorList[trial]=(experiment.blockElist(errorList))
                    trial = trial + 1
                trial = 0
                errorList = []
...
#            #print bErrorList
    ...
    def presentTrials(self,experiment,As,Ps,criterion,errorList,trial,B_file,block):
            t = 0
            FB = 0
            fs = 0
            total = 0
            ...
            while tTime < experiment.trialLen:
                ...
                fs=fs+1
            trial = trial + 1
            totalPhrase = 'You earned ' + str(total*experiment.eRate) + ' GBP'
            ...
            while curbw - startbw < experiment.trialWait:
                if curbw - startbw < experiment.trialWait - 2:
                    self.totalPhraseL.draw()
                    experiment.myWin.flip()
                    curbw = time.time()
                else:
