...
class Exp:
    def __init__(self):
        ...
        self.trialLen = 14 
        self.trialWait = 3
        self.path = './data/HV_'+str(self.subjectID)+'.Cont/'
        ...
        print self.run
        self.FBprops = [[0.45,0.45,0.46],[0.46,0.46,0.47],[0.49,0.50,0.50],[0.50,0.51,0.52],[0.53,0.54,0.55],[0.49,0.50,0.50],[0.50,0.51,0.52],[0.53,0.54,0.55]]
    ...
    def genSubjSine(self):
            ...
            if RR == 1:
                ...
                A = [0.7922822795267915, 0.3447533857022764, 0.692, 0.6185935466858701] [0.01, 0.31, 1.14, 1.76]
...
class expPresentation:
    ...
    def compareErr2Crit(self,experiment,blk,t,fblist):
        t = int(t*10)
    ...
    def showInst(self,experiment):
        ...
        if experiment.run in ['001','006','007','008']:
            ...
            self.InstructionRunL1 = visual.TextStim(win=experiment.myWin, text='Track the blue circle with the white dot.  Squeeze the handle to move up.', wrapWidth=10.0, font='Arial',
            pos=[0, 1], height=1.0, color=[1.000,1.000,1.000], colorSpace='rgb', opacity=1, depth=0.0)
            self.InstructionRunL2 = visual.TextStim(win=experiment.myWin, text='Your payment will be determined basedo on your tracking performance. The red dot does not indicate your performance.', wrapWidth=10.0, font='Arial',
            pos=[0, 0], height=1.0, color=[1.000,1.000,1.000], colorSpace='rgb', opacity=1, depth=0.0)
            self.InstructionRunL3 = visual.TextStim(win=experiment.myWin, text='Press Space to continue.', wrapWidth=10.0, font='Arial',
            pos=[0, -1], height=1.0, color=[1.000,1.000,1.000], colorSpace='rgb', opacity=1, depth=0.0)
        while showInst == True:
    ...
    def presentTrials(self,experiment,As,Ps,criterion,errorList,trial,B_file,block):
            ...
            fblist = [random.random() for i in range(10000)]
            trialtime = time.time()
            cur_t = time.time()
            tTime = cur_t - trialtime
            errorList=[0]*10000
            while tTime < experiment.trialLen:
                ...
                tTime = cur_t - trialtime
                currentError,errorList = experiment.compareCur2Tar(errorList,cur_y,Y,fs)
                FB = self.compareErr2Crit(experiment,block,tTime,fblist)
                if t == 1:
                    total = total + 1
                B_writer = csv.writer(B_file)
                B_writer.writerow([trial,criterion,Y,cur_y,currentError,Y-cur_y,FB,tTime])
