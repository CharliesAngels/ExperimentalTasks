...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.81.03), Mon Feb 13 21:59:13 2017
...
expInfo = {u'6 Gender (M/F)': u'', u'2 cond': u'', u'1 participant': u'', u'4 numTrials': u'96', u'3 seq': u'', u'5 Age': u'', u'7 Music experience (1-7)': u''}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'/Users/steelad/Desktop/tasks.Applications/lbSRTT/lbSRTTFeedback.psyexp',
...
# Initialize components for Routine "questWait"
questWaitClock = core.Clock()

qWait = visual.TextStim(win=win, ori=0, name='qWait',
    text='Press space to move on to the questionaire',    font='Arial',
    pos=[0, 0], height=1, wrapWidth=None,
    color='white', colorSpace='rgb', opacity=1,
    depth=-1.0)
...
quest = visual.TextStim(win=win, ori=0, name='quest',
    text='default text',    font='Arial',
    pos=[0, 0], height=1, wrapWidth=None,
    color='white', colorSpace='rgb', opacity=1,
    depth=0.0)
rating = visual.RatingScale(win=win, name='rating', marker='circle', size=1.0, pos=[0.0, -0.4], low=0, high=1, precision=100, showValue=False, markerExpansion=0, scale='')
...
Blocks = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=u'/Users/steelad/Desktop/tasks.Applications/lbSRTT/lbSRTTFeedback.psyexp',
    trialList=data.importConditions('lbBlockOrder.xlsx'),
...
for thisBlock in Blocks:
    ...
    #------Prepare to start Routine "questWait"-------
    t = 0
    questWaitClock.reset()  # clock 
    frameN = -1
    # update component parameters for each repeat
    if BlockNum > 3 and BlockNum < 13:
        Q_list = 'QList.xlsx'
    else:
        Q_list = 'QList_2.xlsx'
    qMoveOn = event.BuilderKeyResponse()  # create an object of type KeyResponse
    qMoveOn.status = NOT_STARTED
    # keep track of which components have finished
    questWaitComponents = []
    questWaitComponents.append(qWait)
    questWaitComponents.append(qMoveOn)
    for thisComponent in questWaitComponents:
        if hasattr(thisComponent, 'status'):
            thisComponent.status = NOT_STARTED
    
    #-------Start Routine "questWait"-------
    continueRoutine = True
    while continueRoutine:
        # get current time
        t = questWaitClock.getTime()
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        
        
        # *qWait* updates
        if t >= 0.0 and qWait.status == NOT_STARTED:
            # keep track of start time/frame for later
            qWait.tStart = t  # underestimates by a little under one frame
            qWait.frameNStart = frameN  # exact frame index
            qWait.setAutoDraw(True)
        if qWait.status == STARTED and t >= (0.0 + (1.0-win.monitorFramePeriod*0.75)): #most of one frame period left
            qWait.setAutoDraw(False)
        
        # *qMoveOn* updates
        if t >= 0.0 and qMoveOn.status == NOT_STARTED:
            # keep track of start time/frame for later
            qMoveOn.tStart = t  # underestimates by a little under one frame
            qMoveOn.frameNStart = frameN  # exact frame index
            qMoveOn.status = STARTED
            # keyboard checking is just starting
            event.clearEvents(eventType='keyboard')
        if qMoveOn.status == STARTED:
            theseKeys = event.getKeys(keyList=['space'])
    ...
    # set up handler to look after randomisation of conditions etc
    questions = data.TrialHandler(nReps=1, method='random', 
        extraInfo=expInfo, originPath=u'/Users/steelad/Desktop/tasks.Applications/lbSRTT/lbSRTTFeedback.psyexp',
        trialList=data.importConditions(Q_list),
    ...
    for thisQuestion in questions:
        ...
        t = 0
        QuestionaireClock.reset()  # clock 
        frameN = -1
        # update component parameters for each repeat
        quest.setText(Question)
        rating.reset()
        # keep track of which components have finished
        QuestionaireComponents = []
        QuestionaireComponents.append(quest)
        QuestionaireComponents.append(rating)
        for thisComponent in QuestionaireComponents:
            if hasattr(thisComponent, 'status'):
                thisComponent.status = NOT_STARTED
        
        #-------Start Routine "Questionaire"-------
        continueRoutine = True
        while continueRoutine:
            # get current time
            ...
            if t >= 0.0 and quest.status == NOT_STARTED:
                # keep track of start time/frame for later
                quest.tStart = t  # underestimates by a little under one frame
    ...
    elif BlockNum > 3 and BlockNum < 13:
    #    if cond == 'Rew' or cond == 'Pun':
     #       breakPhrase =  str("Nice job.  Take a breather. \n \nYou have earned $%s\n\nPress space to move on." % (e_string))
      #  else:
    #        breakPhrase = str("Nice job.  Take a breather.\n\nYou have earned money.\n\nPress space to move on.")
        breakPhrase = str("Nice job.  Take a breather.\n\nYou are in the earning phase of the experiment.\n\nContinue to respond as fast and accurately as possible.\n\nPress space to move on.")
    elif BlockNum == 13:
